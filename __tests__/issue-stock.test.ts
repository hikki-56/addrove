...
class MockStockRepo implements IStockRepository {
  ...
  movements = {
    ...
    getBalance: async (pId: string, wId: string, lId: string) => {
      return this.balances[`${pId}:${wId}:${lId}`] || 0;
    },
...
describe("issueStock Use Case", () => {
  ...
  test("should successfully issue stock when balance is sufficient", async () => {
    const input = IssueStockSchema.parse({
      warehouse_id: "wh-1",
      document_date: "2026-08-08",
      reference_no: "ISS-001",
      note: "เบิกสินค้า",
      idempotency_key: "idem-iss-1",
      lines: [
        { product_id: "prod-001", location_id: "loc-A", qty: 20 },
      ],
    });

    const doc = await issueStock({ repo }, { ...input, user_id: "user-1" });
    expect(doc).toBeDefined();
    expect(doc.document_type).toBe("ISSUE");
    expect(doc.status).toBe("POSTED");
    expect(repo.movementsList.length).toBe(1);
    expect(repo.movementsList[0].qty_change).toBe(-20);
    expect(repo.movementsList[0].movement_type).toBe("ISSUE_OUT");
  });

  test("should throw InsufficientStockError when balance is less than requested quantity", async () => {
    const input = IssueStockSchema.parse({
      warehouse_id: "wh-1",
      document_date: "2026-08-08",
      idempotency_key: "idem-iss-over",
      lines: [
        { product_id: "prod-001", location_id: "loc-A", qty: 999 },
      ],
    });

    await expect(issueStock({ repo }, { ...input, user_id: "user-1" })).rejects.toThrow(
      InsufficientStockError
    );
  });

  test("should throw StockConflictError when idempotency key is duplicate", async () => {
    repo.usedKeys.add("idem-dup-iss");

    const input = IssueStockSchema.parse({
      warehouse_id: "wh-1",
      document_date: "2026-08-08",
      idempotency_key: "idem-dup-iss",
      lines: [
        { product_id: "prod-001", location_id: "loc-A", qty: 5 },
      ],
    });
