...
describe("moveStock Use Case", () => {
  ...
  test("should successfully move stock between locations within warehouse", async () => {
    const input = MoveStockSchema.parse({
      warehouse_id: "wh-1",
      product_id: "prod-001",
      ...
      reference_no: "MOV-001",
      note: "ย้ายสินค้า",
      idempotency_key: "idem-mov-1",
    });

    const doc = await moveStock({ repo }, { ...input, user_id: "user-1" });
    expect(doc).toBeDefined();
    expect(doc.document_type).toBe("MOVE");
    expect(doc.status).toBe("POSTED");
    expect(repo.movementsList.length).toBe(2);
    expect(repo.movementsList[0].qty_change).toBe(-20);
    expect(repo.movementsList[0].movement_type).toBe("MOVE_OUT");
    expect(repo.movementsList[1].qty_change).toBe(20);
    expect(repo.movementsList[1].movement_type).toBe("MOVE_IN");
  });

  test("should throw InsufficientStockError when source location does not have enough stock", async () => {
    const input = MoveStockSchema.parse({
      warehouse_id: "wh-1",
      product_id: "prod-001",
      from_location_id: "loc-A",
      ...
      qty: 999,
      document_date: "2026-08-08",
      idempotency_key: "idem-mov-over",
    });

    await expect(moveStock({ repo }, { ...input, user_id: "user-1" })).rejects.toThrow(
