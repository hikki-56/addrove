...
export default function WarehouseQrPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 w-full max-w-full pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-5 print:hidden">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            สแกนเพื่อเปิดหน้าล็อกอินพนักงาน (PIN) และสลับเข้าสู่ระบบโกดังที่เลือกโดยอัตโนมัติ
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>พิมพ์ป้าย QR ทั้งหมด 5 โกดัง</span>
          </button>
        </div>
      </div>

      {/* Target Action & Base URL Selector (Hidden when printing) */}
      <div className="glass-card rounded-xl p-4 space-y-4 print:hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 print:grid-cols-2 print:gap-6 print:text-black">
        {WAREHOUSES.map((wh) => {
          ...
          return (
            <div
              key={wh.id}
              className="glass-card rounded-2xl p-6 border border-white/[0.1] bg-[#111118] flex flex-col items-center justify-between text-center space-y-4 shadow-xl hover:border-indigo-500/40 transition-all print:bg-white print:border-2 print:border-slate-800 print:text-slate-900 print:shadow-none"
            >
              {/* Card Header */}
              <div className="space-y-1 w-full border-b border-white/[0.08] pb-3 print:border-slate-300">
                <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 print:bg-slate-100 print:text-slate-900 print:border-slate-400">
                  {wh.code}
                </span>
                <h2 className="text-xl font-bold text-slate-100 print:text-slate-900 mt-1">{wh.name}</h2>
                <p className="text-xs text-slate-400 print:text-slate-600 font-medium">{wh.desc}</p>
              </div>

              {/* QR Code Container */}
              <div className="bg-white p-4 rounded-2xl border border-white/20 shadow-md flex items-center justify-center min-h-[200px] min-w-[200px] print:border-slate-400">
                {qrDataUrl ? (
              ...
              <div className="w-full text-center space-y-1">
                <p className="text-sm font-bold text-amber-300 print:text-slate-900">
                  สแกนเพื่อ: {actionObj.label} ({wh.name})
                </p>
                <Link
                  href={fullTargetUrl}
                  target="_blank"
                  className="text-xs text-indigo-400 hover:underline font-mono truncate block px-2 print:text-slate-600"
                >
                  {fullTargetUrl}
                </Link>
              </div>
              ...
              <div className="w-full pt-2 flex gap-2 print:hidden">
                <button
                  type="button"
