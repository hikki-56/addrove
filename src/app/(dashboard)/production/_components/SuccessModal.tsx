...
export default function SuccessModal({ orderNo, onClose }: SuccessModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl max-w-sm w-full p-6 text-center space-y-4 shadow-2xl border border-[#E8ECEA]">
        <div className="w-16 h-16 rounded-2xl bg-[#DFEDE6] text-[#053425] flex items-center justify-center text-3xl mx-auto shadow-xs">
          🎉
        </div>
        ...
        <div className="flex flex-col gap-2 pt-2">
          <Link
            ...
            className="w-full py-3.5 rounded-xl bg-[#06402B] hover:bg-[#053425] text-white font-bold text-sm transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-[#06402B]/20 cursor-pointer active:scale-95"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm transition-all cursor-pointer border border-[#E8ECEA] active:scale-95"
          >
