  ...
  return (
    <div className="relative flex-1 min-w-0" ref={ref}>
      <button
        ...
        title={title}
        className="w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 border border-[#E8ECEA] text-slate-800 text-xs sm:text-sm font-semibold transition-all cursor-pointer shadow-2xs focus:outline-none focus:border-[#0F5C3F] focus:bg-white focus:ring-2 focus:ring-[#0F5C3F]/20"
      >
      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-1.5 w-full min-w-[160px] bg-white border border-[#E8ECEA] rounded-xl shadow-xl z-50 max-h-[160px] overflow-y-auto divide-y divide-[#EEF1EF] py-1">
          {options.map((opt) => {
            ...
            return (
              ...
              >
                ...
                {isSelected && <span className="text-[#06402B] text-xs font-bold ml-1.5 shrink-0">✓</span>}
              </button>
            );
          })}
        </div>
    ...
    return (
      ...
      >
        <h2 className="text-3xl sm:text-4xl md:text-[2.6rem] font-black text-slate-900 tracking-wide font-mono select-none flex items-center justify-center gap-2 sm:gap-3 flex-nowrap w-full whitespace-nowrap overflow-hidden">
          ...
          {arrowDir === "up" && (
            <svg className="w-6 h-6 sm:w-8 sm:h-8 text-[#06402B] inline-block shrink-0 fill-current" viewBox="0 0 24 24" aria-label="B: ชี้ขึ้น">
        ...
        <div className="w-full flex items-center justify-center">
          <div className="w-full max-w-[380px] sm:max-w-[420px] bg-white rounded-2xl border border-[#D5DDD9] px-4 py-3 sm:px-6 sm:py-3.5 flex items-center justify-center shadow-2xs">
            <BarcodeSvg
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto space-y-6 w-full max-w-full pb-12 print:hidden">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-[#E8ECEA] shadow-xs">
            ...
            <div className="sm:col-span-2 lg:col-span-5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-50 border border-[#E8ECEA] text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#0F5C3F] focus:bg-white focus:ring-2 focus:ring-[#0F5C3F]/20 text-xs sm:text-sm font-medium transition-all shadow-2xs"
                />
        ...
        {loading ? (
          <div className="py-20 text-center space-y-4">
            <div className="w-10 h-10 border-4 border-[#06402B] border-t-transparent rounded-full animate-spin mx-auto" />
            <p className="text-slate-500 text-sm font-medium">กำลังโหลดข้อมูลชั้นวางสินค้า...</p>
          </div>
        ) : filteredItems.length === 0 ? (
          <div className="py-20 text-center bg-white border border-[#E8ECEA] shadow-xs rounded-2xl p-8 space-y-3">
          ...
          <div className="space-y-8">
            {groupedByLevel.map((group) => (
              <div key={group.levelName} className="space-y-3.5">
                <div className="flex items-center justify-between border-b border-[#E8ECEA] pb-2">
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#06402B]" />
