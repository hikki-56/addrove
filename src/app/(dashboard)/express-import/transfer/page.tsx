...
export default function ExpressTransferPage() {
  ...
  const fetchMovements = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      const prods: any[] = Array.isArray(prodJson?.data)
        ...
        ? prodJson.data.data
        : [];
      // คง state เดิมเมื่อข้อมูล catalog ไม่เปลี่ยน เพื่อไม่ให้ polling ทั้งหน้า re-render ทุก 8 วิ
      setCatalogProducts((prev) => (isSameJson(prev, prods) ? prev : prods));
      ...
      if (expTrfJson && expTrfJson.success && Array.isArray(expTrfJson.data)) {
        ...
        // คง state เดิมเมื่อรายการไม่เปลี่ยน (ลด re-render ตอน polling)
        setMovements((prev) => (isSameJson(prev, incoming) ? prev : incoming));
        ...
        if (movJson?.success && Array.isArray(movJson.data?.data)) {
          ...
          setMovements((prev) => (isSameJson(prev, transferMovements) ? prev : transferMovements));
        }
      }
    } catch (e) {
  ...
  return (
    <div className="w-full max-w-full space-y-5 pb-12">
      {/* Top Action Toolbar */}
      <div className="flex flex-wrap items-center justify-end gap-2 bg-white p-3.5 sm:p-4 rounded-2xl border border-[#E8ECEA] shadow-sm print:hidden">
        <button
          type="button"
