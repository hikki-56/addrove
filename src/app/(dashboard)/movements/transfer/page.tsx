...
export default function TransferPage() {
  ...
  return (
    <div className={`${containerWidth} mx-auto w-full px-3 sm:px-6 py-2 sm:py-4 space-y-4`}>
      {isApprover ? (
        /* Dedicated Approver Banner */
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="px-3 py-1 rounded-full text-xs font-semibold bg-amber-500 text-slate-950 shadow-xs shrink-0">
            รออนุมัติ {waitingApprovalTasks.length} รายการ
          </span>
        </div>
      ) : (
        /* Segmented Switch Bar: สลับไปทำรายการ / สร้างใบย้าย / รออนุมัติ */
        <div className="flex items-center p-1 bg-slate-100 border border-slate-200 rounded-xl gap-1 shadow-xs">
          <button
            type="button"
            onClick={() => setActiveMode("ADMIN_CREATE")}
            className={`flex-1 py-2 px-3 rounded-lg font-semibold text-xs sm:text-sm transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
          ...
          {(isAdmin || waitingApprovalTasks.length > 0) && (
            ...
            >
              ...
              {waitingApprovalTasks.length > 0 && (
                <span className="px-1.5 py-0.2 rounded-full text-[11px] font-bold bg-amber-500 text-slate-950">
