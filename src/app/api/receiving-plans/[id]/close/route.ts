...
import { closeReceivingPlan, mapStockErrorToResponse, CloseReceivingPlanSchema } from "@/lib/services/stock";
import {
  successResponse,
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = CloseReceivingPlanSchema.safeParse(body);

    const view = await closeReceivingPlan(
      { repo: getRepository() },
      {
        plan_id: id,
    ...
    return successResponse(view, "ปิดแผนรับสินค้าเรียบร้อยแล้ว");
  } catch (e) {
