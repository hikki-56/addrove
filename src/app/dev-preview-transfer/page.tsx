        ...
        <div className="grid grid-cols-3 p-1.5 bg-slate-100 border border-[#E8ECEA] rounded-2xl gap-1.5 sm:gap-2 shadow-xs items-stretch">
          {["สร้างใบเบิกสินค้า", "รายการที่ต้องไปเบิก", "รออนุมัติ"].map((label, i) => (
            ...
            >
              ...
              {i === 1 && <span className="absolute top-1.5 right-1.5 px-2 py-0.5 rounded-full text-sm font-black bg-rose-600 text-white shadow-xs">3</span>}
              {i === 2 && <span className="absolute top-1.5 right-1.5 px-2 py-0.5 rounded-full text-sm font-black bg-amber-500 text-slate-950 shadow-xs">2</span>}
            </button>
          ))}
        </div>
        <TransferForm
